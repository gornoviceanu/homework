...
// metodele
	static void viewAvailable() {
		for (Video video : videos) {
			System.out.println(video.toString() + " // Average rating: " + video.rating + " // Number of views: " + video.i + " // Available: " + video.available);			
		}
	}
	...
	static void checkOut(String title) {
		  for (Video video : videos) {
			 ...
			 } else if ((video.title).equals(title) && video.available == false) {
			            System.out.println("Title is currently not available.");
		   } 
		 } 
	}
	
	// in metoda checkIn am implementat metoda receiveRating pentru ca m-am gandit ca nu poti lasa un Rating fara sa vezi filmul. As fi putut sa o adaug individual in meniu dar m-am gandit ca e mai bine asa.
	static void checkIn(String title) {
		for (Video video : videos) {
			if((video.title).equals(title) && video.available == false) {
				video.available = true;
				System.out.println("Thank you for returning the video. Please leave an integer rating between 1-10 for the movie: ");
